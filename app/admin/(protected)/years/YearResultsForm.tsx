...
type PositionRow = { id?: string; position: number; name: string; wine: string; points: string };
...
export function YearResultsForm({ yearId }: YearResultsFormProps) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (wh.data) setWhite(wh.data.map((x) => ({ id: x.id, position: x.position ?? 0, name: x.name ?? "", wine: x.wine ?? "", points: x.points ?? "" })));
      if (r.data) setRed(r.data.map((x) => ({ id: x.id, position: x.position ?? 0, name: x.name ?? "", wine: x.wine ?? "", points: x.points ?? "" })));
      setLoading(false);
    })();
  }, [yearId]);
  ...
  return (
    <form onSubmit={handleSubmit} className="max-w-3xl space-y-6">
      ...
      <div className={sectionCls}>
        ...
        <button type="button" onClick={() => setWhite((p) => [...p, { position: p.length + 1, name: "", wine: "", points: "" }])} className="mt-2 text-sm text-[#7A1E2C] font-medium hover:underline">
          + Přidat řádek
        </button>
      </div>
      ...
      <div className={sectionCls}>
        ...
        <button type="button" onClick={() => setRed((p) => [...p, { position: p.length + 1, name: "", wine: "", points: "" }])} className="mt-2 text-sm text-[#7A1E2C] font-medium hover:underline">
          + Přidat řádek
        </button>
      </div>
